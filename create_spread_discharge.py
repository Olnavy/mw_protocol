...
Please make sure you update the collection boxes and spreading regions json files to suite your needs.
"""
...
overlapping = False  # Whether to apply overlapping freshwater points
overlapping_max_radius = 5  # Maximum radius for overlapping freshwater points

output_name = 'glac1d_freshwater_nemo_spread.nc'  # Name of the output file
...
## Overlapping - Default No
if overlapping:
    # Apply overlapping freshwater points
    ds_shifted = mw.overlapping(fw_routed_nemo, lsm, radius_max=overlapping_max_radius, verbose=True)
else:
    ds_shifted = fw_routed_nemo

## Spreading
ds_spread = mw.spreading(ds_shifted, collection_boxes, spreading_regions, grid_ref).to_dataset()

## Saving
ds_spread.to_netcdf(f"{outputs_folder}/{output_name}", mode='w', format='NETCDF4_CLASSIC')
